...
_RE_COLUNA_CALCULADA = re.compile(r"(?i)%|\bvar\b|varia[çc][ãa]o|^\W*c[óo]digo\W*$|^\W*c[óo]d\.?\W*$|^\W*n[úu]mero\W*$|^\W*n[°º]\W*$|^\W*classifica[çc][ãa]o\W*$|^\W*empresa:?\W*$|^\W*c\.?n\.?p\.?j\.?:?\W*$|^\W*per[íi]odo:?\W*$")
...
def _find_free_period_header_row(ws, max_scan_rows: int = 10, min_periodos: int = 2, max_scan_dados: int = 30):
    ...
    for idx, row in enumerate(linhas_cache[:max_scan_rows]):
        ...
        if len(candidatos) >= min_periodos:
            # Achado real (arquivo real "DRE e Faturamento Accord"): às
            # vezes as colunas candidatas não são períodos de tempo
            # diferentes — são SUB-COMPONENTES que somam um único total
            # ("Contabilidade" + "Rec. Crédito" = "Total"). Tratar cada
            # uma como "mês" distorce tudo (usar só a coluna parcial
            # "Rec. Crédito" como se fosse um período inteiro subestima a
            # receita daquele "mês" pela metade). Quando existe uma
            # coluna claramente chamada "Total"/"Consolidado"/"Geral"
            # entre os candidatos E há mais de uma coluna candidata,
            # prefere usar SÓ essa — ela já é a soma que interessa, as
            # outras são as partes que a compõem.
            candidatos_total = {
...
def _extrair_hierarquia_por_colunas(ws, header_row: int, meses_col: dict, colunas_hier: list[int], max_data_rows: int) -> dict:
    ...
    GENERALIZAÇÃO PRA N NÍVEIS (achado real, arquivo real da GATTI
    Contabilidade): a versão anterior só usava 2 colunas (a mais
    agregada como "raiz", a mais específica como "detalhe") — um plano
    de contas com hierarquia GENUINAMENTE profunda (5 níveis reais:
    "CONTAS DE RESULTADO" > "RECEITAS OPERACIONAIS" > "RECEITA BRUTA DE
    VENDAS E SERVIÇOS" > "RECEITA DE PRESTAÇÃO DE SERVIÇOS" > "SERVIÇOS
    PRESTADOS") perdia os 3 níveis do meio inteiros — exatamente onde
    estavam as categorias que a Margem Bruta precisa ("RECEITA BRUTA...",
    "DESPESAS COM PESSOAL"). Generaliza processando CADA nível
    (colunas_por_generalidade, do mais agregado pro mais específico),
    aplicando a MESMA proteção de dupla contagem já usada aqui (raiz com
    valor próprio vs. soma de filhas — achado real Grupo Roma) entre
    cada par de níveis ADJACENTES, e o mesmo forward-fill com reset em
    cascata. Um rótulo só entra no resultado final na primeira vez (nível
    mais agregado) que aparecer — isso evita contar a mesma árvore 2x
    (pai + filho) quando um regex de categoria bate em mais de um
    nível."""
    ...
    # cria valor novo), então decidir a ordem aqui, antes de preencher,
    # não muda o resultado dessa decisão — mas permite o fix de reset
    # abaixo, que PRECISA saber a ordem de generalidade antes de preencher.
    ...
    # Forward-fill por coluna, em CASCATA — cada nível reseta o PRÓXIMO
    # nível (mais específico) quando tem valor novo na linha. Achado
    # real em 28/08 (deal Grupo Roma — PDF de auditoria financeira
    ...
    # raiz — isso quebrava o fix de dupla contagem logo abaixo (que
    # depende de reconhecer corretamente qual linha é "a própria raiz").
    # Regra: sempre que um nível tiver um valor próprio nesta linha (não
    # herdado), TODOS os níveis mais específicos que ele têm seu detalhe
    # herdado descartado — pertencem à categoria anterior, não fazem
    # sentido carregar pra cá.
    ultimo_valor = {c: None for c in colunas_hier}
    for l in linhas_cruas:
        for idx, c in enumerate(colunas_por_generalidade):
            if l["textos"][c] is not None:
                for c_mais_especifico in colunas_por_generalidade[idx + 1:]:
                    ultimo_valor[c_mais_especifico] = None
                break
        for c in colunas_hier:
            if l["textos"][c] is not None:
    ...
    # Pra CADA nível (do mais agregado pro mais específico), monta as
    # raízes daquele nível — com a MESMA proteção de dupla contagem já
    # usada aqui (achado real Grupo Roma: quando a linha que DEFINE a
    # raiz já traz valor próprio, esse é o total pronto da categoria;
    # linhas-filha que vierem depois são só detalhamento, nunca somadas
    # por cima), comparando cada nível com o PRÓXIMO mais específico
    # (não só com o extremo mais específico de todos).
    raizes_dict: dict[str, dict] = {}
    for idx, col in enumerate(colunas_por_generalidade):
        col_proximo_nivel = colunas_por_generalidade[idx + 1] if idx + 1 < len(colunas_por_generalidade) else col
        agregado_do_nivel: dict[str, dict] = {}
        raizes_com_total_proprio_do_nivel: set[str] = set()
        for l in linhas_cruas:
            raiz = l["textos"][col]
            detalhe = l["textos"][col_proximo_nivel]
            if raiz is None:
                continue
            if raiz not in agregado_do_nivel:
                agregado_do_nivel[raiz] = {"rotulo": raiz, "valores": {}, "detalhes": []}
            ...
            if eh_linha_propria_da_raiz and l["valores"]:
                for k, v in l["valores"].items():
                    agregado_do_nivel[raiz]["valores"][k] = agregado_do_nivel[raiz]["valores"].get(k, 0) + v
                raizes_com_total_proprio_do_nivel.add(raiz)
            elif raiz not in raizes_com_total_proprio_do_nivel:
                for k, v in l["valores"].items():
                    agregado_do_nivel[raiz]["valores"][k] = agregado_do_nivel[raiz]["valores"].get(k, 0) + v
            if detalhe and detalhe != raiz and detalhe not in agregado_do_nivel[raiz]["detalhes"]:
                agregado_do_nivel[raiz]["detalhes"].append(detalhe)
        # Só entra no resultado final quem ainda não apareceu num nível
        # MAIS AGREGADO já processado — evita contar a mesma árvore 2x
        # (pai + filho) quando um regex de categoria bate em mais de um
        # nível (ex.: "receita" batendo tanto o nível macro quanto um
        # nível intermediário mais específico da mesma árvore).
...
_RE_MB_DESPESA_PESSOAL = re.compile(r"(?i)despesa.*pessoal|folha[\s_]*de[\s_]*pagamento|custo.*folha|\bfolha\b|despesa[\s\w]*\brh\b|\brh\b[\s\w]*(fixa|vari[áa]vel|direto)|\bpessoal\b")
...
def _achar_linha_por_padrao(fonte: dict, padrao: re.Pattern, formato: str, mapa_categorias_extra: dict | None = None, categoria_alvo: str | None = None) -> dict | None:
    ...
    FALLBACK POR IA (achado real testando ~30 DREs reais): quando o
    regex não bate NADA, e `mapa_categorias_extra` foi passado (vem de
    `classificar_categoria_financeira_via_ia`, em run_agent.py — aqui em
    baixo o módulo não faz chamada de IA nenhuma, só CONSOME um
    ...
    existirem, porque a categoria pode estar diluída (ex.: receita
    detalhada por cliente individual, sem linha agregada — achado real
    no SKZ Oberle)."""
...
def _corrigir_convencao_sinal_receita(receita: dict) -> dict:
    """Corrige convenção de sinal contábil formal (achado real testando o
    arquivo real da GATTI Contabilidade — abas literalmente chamadas
    ...
    Mesma ideia já usada em `calcular_resultado_de_hierarquia` (que
    detecta a convenção pela soma de despesa) — aqui aplicada
    diretamente sobre a linha de RECEITA já encontrada, nas 3 funções
    que a usam (`extrair_margem_bruta_de_dre`, `montar_mini_dre`,
    `montar_tabela_viabilidade_financeira`), que antes não tinham essa
    correção — só a função "aproximação grosseira" tinha, deixando a
    tabela que alimenta o Excel/PPT/contexto dos outros agentes vazia ou
    incorreta mesmo quando o Bloco A (via `calcular_resultado_de_hierarquia`)
    já calculava certo. Detecta pela soma agregada — receita de negócio
    saudável quase nunca é negativa na soma total — nunca aplica
    cegamente numa célula isolada, só quando a SOMA do período inteiro
    vier negativa."""
    if sum(v for v in receita.values() if isinstance(v, (int, float)) and not isinstance(v, bool)) < 0:
...
def rotulos_sem_categoria_financeira(dre_estruturada: dict | None, hierarquia: dict | None) -> list[str]:
    ...
    fora linhas de resultado/subtotal calculado e categorias já
    resolvidas pelo `DRE_CATEGORIAS`/sinal mecânico/regex de despesa.

    No caminho FINO (`dre_estruturada`), cada linha da planilha já vem
    individual (incluindo linha de DETALHE dentro de um bloco maior) —
    a maioria NÃO tem sinal mecânico próprio (só o subtotal/raiz costuma
    ter "(-)" explícito). Exigir sinal aqui excluiria exatamente as
    linhas de detalhe que precisam de classificação — por isso entra
    tudo que NÃO tem sinal mecânico nenhum (nem despesa, nem receita,
    nem resultado): o subtotal/raiz (que TEM sinal) fica de fora de
    propósito, pra não contar o mesmo dinheiro 2x (pai + filho)."""
    candidatos = []
...
def rotulos_candidatos_receita(dre_estruturada: dict | None, hierarquia: dict | None) -> list[str]:
    ...
    RECEITA — achado real (SKZ Oberle, versão antiga): a receita vem
    detalhada por CLIENTE individual ("Honorários Contábeis", "Clientes
    - Serviços Prestados"...), sem nenhuma linha agregada "Receita
    Bruta"/"Total" — `_RE_MB_RECEITA_BRUTA` nunca bate nada disso.
    Retorna candidatos sem sinal mecânico próprio (mesma lógica de
    detalhe-sem-sinal-próprio de `rotulos_sem_categoria_financeira`) que
    ainda não bateram `_RE_MB_RECEITA_BRUTA`/`_RE_MB_RECEITA_LIQUIDA`."""
    candidatos = []
    for fonte, formato in ((dre_estruturada, "fino"), ((hierarquia or {}).get("raizes_classificadas"), "hierarquia")):
        if not fonte:
            continue
...
def extrair_margem_bruta_de_dre(dre_estruturada: dict | None, hierarquia: dict | None, regime_tributario: str | None, mapa_categorias_extra: dict | None = None) -> dict | None:
    ...
    for fonte, formato in fontes:
        ...
        if margens_por_periodo:
            ...
            confianca = "alta"
            # Acha "margem pronta" SEMPRE (não só quando o cálculo já
            # parece implausível) — achado real (DRE Gerencial 2025): o
            # cálculo por componentes deu 84% (dentro da faixa "0-85%
            # plausível", nunca disparava checagem nenhuma), mas o valor
            # real que a própria fonte já declarava era 53,8% — "(-)
            # PESSOAL" capturava só uma fração do custo direto real
            # (existia também "(-) CSP", não reconhecido). Uma margem
            # "plausível" ainda pode divergir muito do que a fonte já diz
            # — só checar a faixa 0-85% não pega isso.
            candidatos_margem = [
            ...
            for rotulo, valores in candidatos_margem:
                if all(v is None or (isinstance(v, (int, float)) and -1 <= v <= 2) for v in valores.values()) and any(
                    ...
                    if margens_pronta and media_pronta is not None and 0 <= media_pronta <= 85:
                        margem_pronta_valida = margens_pronta
                        break
            if margem_pronta_valida:
                media_pronta = sum(margem_pronta_valida.values()) / len(margem_pronta_valida)
                if not (0 <= media <= 85):
                    # Cálculo por componentes já implausível — a margem
                    # pronta (plausível) substitui direto.
                    return {
                        "margem_bruta_pct_por_periodo": margem_pronta_valida,
                ...
                elif abs(media - media_pronta) > 15:
                    # Achado real (DRE EQUILIBRIO): as duas fontes podem
                    # medir conceitos DIFERENTES de "margem bruta" (uma
                    # já desconta pessoal, outra só desconta impostos) —
                    # não dá pra escolher às cegas qual delas está certa.
                    # Prefere a margem pronta (vem direto da fonte, sem
                    # risco de escolher a linha errada de despesa), mas
                    # marca confiança baixa pra sinalizar a divergência,
                    # em vez de aceitar silenciosamente uma das duas.
                    return {
                        "margem_bruta_pct_por_periodo": margem_pronta_valida,
                        "custo_folha_pct_por_periodo": {},
                        "custo_sistemas_pct_por_periodo": {},
                        ...
                        "motivo_baixa_confianca": (
                            f"margem calculada por componentes ({round(media,1)}%) diverge "
                            f"da margem pronta declarada na fonte ({round(media_pronta,1)}%) "
                            f"por mais de 15 pontos — as duas podem medir conceitos diferentes."
                        ),
                    }
            if not (0 <= media <= 85):
                # Sem margem pronta disponível pra corrigir — a resposta
                # certa não é fingir certeza. Marca confiança baixa, pra
                # quem consome (Bloco A/Excel/PPT) saber que precisa de
                # revisão manual, em vez de reportar um número
                # provavelmente errado como se fosse confiável.
                confianca = "baixa"
            return {
